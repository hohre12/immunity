...
이건 프롬프트 트릭이 아닙니다. 서브에이전트는 별도 컨텍스트에서 실행되며, 파일 경로만 전달받습니다 — 대화 내역, 의도, 배경 정보 없이. 편향은 서브에이전트가 받지 못하는 것에 의해 제거됩니다.

자세한 설정은 [설치 가이드](./docs/installation.md)를 참조하세요.
...
세상에 완벽한 프로젝트는 없습니다. 어떤 프로젝트든 비판적 관점에서 평가하면 반드시 부족한 점이 나옵니다. Immunity도 마찬가지입니다.

비판적 리뷰를 받았을 때 — `/immunity:critic`이든, 동료든, AI든 — **피드백을 그대로 수용하지 말고 반드시 평가하세요.**

비평가의 역할은 문제를 찾는 것입니다. 항상 무언가를 찾아냅니다. 하지만 모든 발견이 행동을 요구하는 것은 아닙니다. 어떤 발견은 프로젝트의 핵심 정체성과 충돌합니다. 어떤 발견은 당신의 맥락에서는 중요하지 않은 것을 최적화합니다. 어떤 발견은 기술적으로는 맞지만 전략적으로는 틀립니다.

**프로젝트의 주인이 최종 판단자여야 합니다.** 비평을 읽고, 이해하고, 그 다음 결정하세요:

- 이 발견이 프로젝트가 지향하는 방향과 일치하는가?
- 이것을 고치면 프로젝트의 핵심 목적이 더 나아지는가, 아니면 희석되는가?
- 이것은 실제 리스크인가, 여기에는 해당하지 않는 이론적 우려인가?

Immunity는 문제를 찾아주는 도구입니다. 결정을 대신 내려주는 도구가 아닙니다. 면역 체계는 위협을 감지하지만, 어떻게 대응할지는 몸이 결정합니다.
